...
def normalize(im):
    ...
    '''
    mean = MEAN if im.is_cuda else MEAN
    std = STD if im.is_cuda else STD
...
def get_colorization_data(im, opt, model, classifier):
    ...
    with torch.no_grad():
        ...
        logits, reg = model(data['L'], torch.zeros_like(hints), torch.full_like(mask, -.5))
...
def compute_loss(opt, y, criterion):
    t = torch.LongTensor([opt.target] * y.shape[0])
    loss = criterion(y, t)
    if not opt.targeted:
        loss *= -1
    return loss
