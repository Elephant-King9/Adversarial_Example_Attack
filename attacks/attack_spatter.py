...
class attack_spatter:
    ...
    def attack(self, image, epsilon, label):
        ...
        c = [(0.65, 0.3, 4, 0.69, 0.6, 0),
             ...
             (0.67, 0.4, 1, 0.65, 1.5, 1)][epsilon - 1]
        image = np.array(image, dtype=np.float32) / 255.

        liquid_layer = np.random.normal(size=image.shape[:2], loc=c[0], scale=c[1])
        ...
        if c[5] == 0:
            ...
            perturbed_image = cv2.cvtColor(np.clip(image + m * color, 0, 1), cv2.COLOR_BGRA2BGR)
            ...
            # mud brown
            color = np.concatenate((63 / 255. * np.ones_like(image[..., :1]),
                                    42 / 255. * np.ones_like(image[..., :1]),
                                    20 / 255. * np.ones_like(image[..., :1])), axis=2)

            color *= m[..., np.newaxis]
            image *= (1 - m[..., np.newaxis])

            perturbed_image = np.clip(image + color, 0, 1)
            perturbed_image = perturbed_image.transpose((2, 0, 1))
            perturbed_image = torch.from_numpy(perturbed_image).float().to(self.config.device)
            perturbed_image = perturbed_image.unsqueeze(0)
            logger.debug(
